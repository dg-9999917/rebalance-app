...
- [x] **작업 2 — 입력칸 유동 폭**: 신규 `autoSizeInput(el)` — `data-min-ch`/`data-max-ch`(각 입력칸에 열 폭 기준으로 부여) 사이에서 `값 길이+1.5`를 `ch` 단위로 적용, `oninput`(타이핑 중)·`onfocus`(콤마 제거 직후)·`onblur`(콤마 재포맷 직후) 세 시점 전부에서 호출해 값 표시가 바뀔 때마다 폭도 갱신, `renderTable()` 렌더 직후에도 전체 입력칸에 1회 실행(초기값이 긴 경우 대비). `width`만 바꾸는 순수 CSS 조작이라 커서 위치·선택 범위에는 영향 없음(별도 안전장치 불필요, 브라우저 특성상 `style.width` 변경이 포커스·선택 상태를 건드리지 않음). "가운데 기준으로 양옆으로"는 `#tbl-wrap tbody td.num:has(.inp-pf) { text-align:center }` 신규 규칙으로 구현(입력칸이 실제로 들어있는 셀만 골라 가운데 정렬, 표시 전용 숫자 셀은 기존 우측 정렬 그대로) — 최대 폭이 열 폭을 넘지 않도록 현재가·평단가·보유수량·설정비율 4개 열의 colgroup 폭도 함께 소폭 확장(110→125, 105→125, 95→115, 130→150px, 종목 열은 무수정)
- [x] **작업 3 — 소수 부분 작게(종합계좌 뷰)**: 신규 `smallDec(str)` — 이미 포맷된 숫자 문자열에서 `/\.\d+/`(첫 소수점+숫자)만 `<span class="dec-sm">`로 감쌈, `.dec-sm { font-size:0.78em }`(요구 범위 75~80% 충족). `renderConsolidatedView()` 안의 `fmtNum`/`fmtDec` 호출부 전체(현재가·평단가·보유수량·현재비중·평가금액·그룹합계·현금·현금합계)를 `smallDec(...)`로 감쌈 — **계산값(fmtNum/fmtDec 자체)은 무수정**, 이미 만들어진 문자열을 감싸기만 함. 정수 값·'—'는 정규식이 매치 안 돼 그대로 반환됨(자동으로 조건 충족). 개별 계좌 표(`renderTable`)·헤더의 `#in-p0`/`#disp-per1`(input value·textContent라 애초에 HTML 삽입 불가능한 자리)는 건드리지 않아 "다른 화면은 건드리지 않음" 충족
- [x] **작업 4 — 줄무늬 대비 강화**: 라이트 테마 짝수 행 배경만 `#f5f6f8` → `#eceef2`로 진하게(다크 테마 규칙은 무수정 — 기존 코드 주석대로 "다크는 미사용이나 보존" 방침 유지)
- [x] **작업 5 — 탭 글자 진하게**: 라이트 테마 `--tab-active`를 파란 액센트(#1a73e8) → `#111318`("#111 수준", `--text`와 동일값이라 기존 토큰 재사용), `--tab-inactive`를 연회색(#9ca3af) → `#454b57`("#444 수준"). `.tab-btn.active`에 `font-weight:800` 추가(기존 전체 탭 공통 600에서 활성 탭만 더 굵게). 다크 테마는 무수정(활성 탭이 원래 파란 액센트로 이미 어두운 배경에서 잘 보이는 배색이라 "확실한 검정"을 그대로 적용하면 오히려 안 보이게 됨 — 라이트 전용으로 판단해 적용)
- [x] **작업 6 — 추천 확인 시점 확장**: 신규 `lastRecoCheckTs`(마지막 확인 시각) + `checkRecoWeightsThrottled()`(1분 이내면 생략) — `document.visibilitychange`(탭 복귀 시) + `setInterval(..., 30*60*1000)`(30분 주기) 둘 다 이 스로틀 래퍼를 호출. 기존 `checkRecoWeights()`(앱 로드 시)와 설정 탭의 수동 `checkRecoNow()`("업데이트 확인" 버튼)도 `lastRecoCheckTs`를 갱신하도록 해 수동 확인 직후 자동 확인이 즉시 또 뜨지 않게 함(쿨다운 공유). 새 버전이면 기존 `evaluateRecoBanner()` 그대로 재사용(신규 배너 로직 없음)
- [x] **작업 7 — 콘솔 "배포된 버전" 갱신 버그 + 메모 표시**:
  — **원인 특정(grep 결과, `index.html` `nextStrategyVersion()`)**: 저장(`s.lastDeployedVersion = payload.version`, `confirmStrategyDeploy` 안)과 재렌더(`renderSettingsTab()` 호출)는 둘 다 정상 동작하고 있었음 — **실제 버그는 배포될 버전 문자열 자체를 계산하는 `nextStrategyVersion(strategy, datePart)` 함수**(당시 `while (strategy.lastDeployedVersion === \`${datePart}-${n}\`) n++;`)에 있었음. 이 while 조건은 `lastDeployedVersion`이 정확히 `date-1`일 때만 한 번 참이 되어 `n`이 2로 올라가고, 그 외에는(예: 이미 `date-2`인 상태) 조건이 즉시 거짓이라 루프가 단 한 번도 돌지 않고 곧장 `date-1`을 반환 — 즉 이 함수는 구조적으로 같은 날짜에 대해 `-1`과 `-2` 두 값만 왔다갔다 할 수 있고 그 이상은 **절대 만들어내지 못했음**. 그 결과 같은 날 3번째 배포부터 버전이 `-1`로 역행해서 GitHub에 실제로 다시 쓰였고(콘솔이 보여준 값도 그 잘못 계산된 값 그대로라 "갱신 안 됨"처럼 보였을 뿐, 저장·렌더 경로 자체는 정상), 실제로 `git log`에서 "배포: 방어형 2026-07-17-1"이 두 번(처음 배포, 그리고 "-2" 배포 이후 세 번째 배포에서 또) 나타나는 것으로 실측 확인됨. 배포 당시 `weights.json`의 "방어형" 프로필 `memo` 필드에 사용자가 직접 `"2026-07-17-3"`이라고 남겨둔 흔적이 남아있어(실제 `version`은 버그로 인해 `"2026-07-17-1"`), 사용자가 의도한 버전과 실제 기록된 버전이 어긋났던 정황과 정확히 일치
  — **수정**: `nextStrategyVersion()`을 `lastDeployedVersion`에서 현재 날짜 접두사 뒤의 접미 번호를 파싱해 `+1` 하도록 재작성(날짜가 다르면 1부터 새로 시작) — 몇 번을 배포하든 항상 정확히 다음 번호로 전진
  — **로드 시 자가 보정(요구사항 2)**: 신규 `reconcileStrategiesWithRemote(remoteProfiles)` — 고급 섹션을 펼칠 때 유령 프로필 조회(`loadGhostProfiles()`, 기존 fix9 기능)가 이미 수행하는 `fetchRemoteProfiles()` 결과를 그대로 재사용해(추가 네트워크 요청 없음) 로컬 `lastDeployedVersion`/`lastDeployedStocks`/`lastDeployedMemo`가 파일과 다르면 파일 기준으로 덮어쓰고 `saveState()` — `strategyStatus()`가 매 렌더마다 `lastDeployedStocks` 기준으로 새로 판정하므로 상태 배지도 자동으로 재판정됨. 위 nextStrategyVersion 버그처럼 어떤 이유로든 로컬이 파일보다 뒤처진 경우 고급 섹션을 여는 순간 스스로 복구됨(개인정보·토큰 없는 일반 가족 계정은 `getGhToken()`이 비어있어 애초에 이 경로를 타지 않음 — 관리자 콘솔 전용 동작)
  — **메모 열 추가(요구사항 3)**: `appData.strategies[].lastDeployedMemo` 신규 필드(생성 시 `''`, 배포 성공 시 `payload.memo` 저장, 위 자가 보정 시 파일 메모로 동기화, `sanitizeLoadedAppData`에도 보존 필드 추가해 드라이브 백업 왕복에도 유지) — `buildAdvancedSection()` 테이블에 "배포된 버전" 옆 "메모" 열 신규(유령 행에도 `p.memo` 표시), 헤더·빈 상태 colspan 5→6 동반 수정
- [x] Node vm 검증(신규 하네스 — top-level `let`을 `globalThis.__exports`로 브리지):
  — `nextStrategyVersion`: 같은 날짜 4연속 배포(-1→-2→-3→-4, 버그 수정 전이라면 3번째에서 -1로 역행했을 지점 포함) 전부 정확한 순증가 확인, 날짜가 바뀌면 -1로 리셋 확인
  — `reconcileStrategiesWithRemote`: 로컬(-1) vs 원격(-3, 메모 포함) 불일치 상태에서 호출 → 변경 감지·버전/메모 모두 파일 기준으로 갱신 확인, `buildAdvancedSection()` 렌더 결과에 메모 열 헤더·갱신된 버전·메모 문자열 전부 반영됨을 확인
  — `checkRecoWeightsThrottled`: 최초 호출은 조회, 1분 이내 재호출은 생략(호출 수 불변), 61초 후 재호출은 다시 조회 — 3단계 전부 확인
  — `smallDec`: 소수부만 감싸짐(`123,456.78`→`123,456<span class="dec-sm">.78</span>`), 정수·`—`는 그대로
  — `autoSizeInput`: 짧은 값→min 클램프, 과도하게 긴 값→max 클램프, 범위 안의 값→`길이+1.5`ch 정확히 반영 — 3가지 경계 케이스 확인
  — `renderTable()`을 USD 모드로 실행해 미국 종목 행에는 `$` 표시, 한국·현금 행/합계는 `원` 표시(달러 기호 없음)로 남는 것을 정규식으로 확인
  — `node --check` 구문 오류 없음, CSS 중괄호 균형(depth 0) 확인
- [x] `git diff`(안전점 `0a194c5` 대비)에서 계산 함수(getBasePrice/calcUnit/priceKRW/avgKRW/evalVal/currWeight/computeCashAmount/syncCashAmount/buildConsolidatedData/restoreFromDrive/migrateAccount*/migrateStock*) 시그니처 전부 grep 결과 없음(무수정) 확인 — `sanitizeLoadedAppData`는 `lastDeployedMemo` 필드 보존 한 줄만 의도적으로 추가(위 사유)
- [x] sw.js CACHE_NAME rebalance-v65 → rebalance-v66
- [ ] **실제 브라우저·정식 주소 확인 필요**(특히 이번 세션은 CSS 위주 변경이 많아 시각 확인이 중요): `:has()` 선택자로 입력칸 셀 가운데 정렬이 실제로 되는지·자동 폭 조절이 자연스러운지, $ 전환이 미국 행에만 적용되는지, 종합계좌 소수부가 실제로 작게 보이는지, 짝수 행·탭 대비가 밝은 화면에서 체감되는지, 화면을 다시 켰을 때/30분 뒤 배너가 실제로 뜨는지, 다음 배포에서 버전이 정말 정상 증가하는지 — 이번 세션은 Node vm 시뮬레이션까지만 수행

## v3 수정 9차 — 배포 안정화·유령 전략·행 선택·커플링 표시 (index.html 정식 앱) — 2026-07-17
- [x] 시작 전 PROGRESS.md·git log 확인(직전 커밋 920429e). `git fetch`로 origin이 로컬에 없는 커밋 2개(가족 실사용 "배포: 방어형/공격형" — weights.json만 변경)를 발견해 `git merge --ff-only`로 반영. 안전점: `git commit --allow-empty -m "v3 수정 9차 작업 전 안전점"`(`091c409`)
- [x] **작업 2 진단(작업 착수 전 먼저 수행, 사용자에게 결과 보고)**: `weights.json`을 직접 조회한 결과 `profiles` 3개 — `test_0000000000000000000000001`(id `s_mro8x1knr1d9`, 버전 `2026-07-17-1`, 종목 30개), `방어형`(`s_mrojlp7cagkj`), `공격형`(`s_mrojlu3zg0va`). fix9.md가 지목한 유령 프로필과 정확히 일치. 로컬 관리자 콘솔의 `strategies` 배열은 브라우저 localStorage라 레포에서는 확인 불가 — 파일 쪽 잔존만 확정. **실제 제거 방법을 사용자에게 확인**: (a) 내가 git으로 weights.json 직접 편집 vs (b) 기능만 구현하고 사용자가 실제 앱에서 버튼으로 제거 — **사용자가 (b) 선택**, 이번 세션은 기능 구현까지만 하고 실제 제거는 사용자가 정식 앱에서 [GitHub에서 내리기]로 수행하기로 함
- [x] **작업 1 — 배포 409 충돌 처리**: `putRemoteProfiles()`가 409 응답을 `err.status=409`로 구분해 던지도록 수정. 신규 공용 헬퍼 `deployProfilesWithRetry(mergeFn, message, maxRetries=2)` — 매 시도마다 `fetchRemoteProfiles()`로 **내용+sha를 새로 GET**한 뒤 `mergeFn`으로 병합해 PUT(모달 표시 시점에 읽어둔 값을 재사용하지 않음), 409면 0.5~1초(500+random(500)ms) 대기 후 최대 2회까지 재시도(총 3회 시도), 재시도할 때도 매번 최신 내용을 다시 읽어 병합해 낡은 내용으로 다른 전략을 덮어쓰지 않음. 최종 실패 시 정확히 "다른 배포와 겹쳤습니다 — 잠시 후 다시 시도해 주세요" 메시지로 통일. `confirmStrategyDeploy()`(배포)·`deleteStrategyFromGithub()`(GitHub에서 삭제) 둘 다 기존 개별 GET/PUT 코드를 이 공용 헬퍼로 교체(로직 중복 제거). [확정] 버튼 비활성화+"배포 중..." 표시는 기존에 이미 있던 동작이라 그대로 유지(재시도 전체 구간 동안 계속 비활성 상태 유지됨 — try/finally가 재시도 루프 전체를 감싸므로 자동으로 보장)
- [x] **작업 2 — 유령 전략 콘솔 표시 + 제거 기능**: `ghostProfilesCache`(null=미조회/[]=조회했으나 없음·실패·토큰없음/[...]=유령 목록) + `ghostProfilesLoading` 신규 상태. `toggleAdvanced()`가 고급 섹션을 펼칠 때(그리고 아직 미조회 상태일 때) `loadGhostProfiles()`를 비동기 트리거 — `fetchRemoteProfiles()`로 GET한 전체 profiles 중 로컬 `appData.strategies`에 없는 id만 필터링. `buildAdvancedSection()`이 전략 테이블 하단에 회색 행으로 추가 렌더(이름 옆 "파일에만 존재" 배지, 커플링·배포버전·상태 칸은 "—", [GitHub에서 내리기] 버튼). `removeGhostProfile(id)`(confirm 확인) → `removeGhostProfileConfirmed()`가 작업1의 `deployProfilesWithRetry`를 그대로 재사용해 해당 프로필만 제거. weights.json이 실제로 바뀔 수 있는 5곳(토큰 저장/삭제, 배포 성공, GitHub 삭제 성공, 콘솔에서만 삭제, 유령 제거 성공)마다 `invalidateGhostProfilesCache()`(또는 토큰삭제는 캐시를 직접 null로) 호출해 캐시를 비우고 고급 섹션이 열려 있으면 즉시 재조회
  — 전략 [삭제] 확인 모달의 "콘솔에서만 삭제" 설명 아래에 "⚠️ 가족의 '전략 변경' 목록에는 계속 보입니다 — 이 콘솔의 [고급]에 '파일에만 존재'로 다시 나타나며..." 경고 줄 신규 추가(요구사항 4번)
  — **이번 잔존 프로필(`test_0000000000000000000000001`) 실제 제거는 사용자 확인에 따라 미수행** — 기능 구현·검증까지만, 사용자가 정식 앱 [고급] 콘솔에서 직접 [GitHub에서 내리기]로 제거 예정
- [x] **작업 3 — 종목 행 클릭 선택**: `index_v2.html`(3630~3639행)의 기존 구현을 그대로 참고해 이식 — 전역 `document.addEventListener('click', handleTableRowClick)`(이름 붙은 함수로 분리해 테스트 가능하게 함), `e.target.closest('#tbl-wrap tbody tr')`로 행 탐색 → `group-hdr`/`subtotal-row`는 제외 → `e.target.closest('input, button, select, .drag-handle')`면 무시(입력칸·±버튼·×삭제·☰드래그 전부 커버 — 드래그핸들만 button/input이 아니라 별도 `.drag-handle` 클래스로 명시 제외) → 단일 선택 토글(`.row-selected` 클래스, 전체 해제 후 재추가/같은 행이면 해제만). CSS `#tbl-wrap tbody tr.row-selected { background: color-mix(in srgb, var(--accent) 15%, var(--surface)) !important; }`(v2와 동일한 색 공식, 줄무늬·호버보다 우선하도록 `!important`) — 종합계좌 뷰(`#tbl-wrap` 안, `data-stock-id` 없는 일반 `<tr>`)에도 동일하게 적용됨(선택은 표시용이라 문제 없음, 계산·저장 무관)
- [x] **작업 4 — 커플링 계좌 💍 표시**: 신규 `isAccountCoupled(accountId)` = `appData.strategies.some(s => s.coupledAccountId === accountId)`. `buildAccountsSection()`의 `.settings-list-name`과 `renderAccountDropdown()`의 계좌 `<option>` 텍스트 둘 다 이름 앞에 `💍 ` 접두(종합계좌의 `📁 ` 접두와 같은 위치 규칙). **즉시 갱신**: `createStrategyV3()`·`saveStrategyEditV3()`(커플링 계좌 재지정 포함)·`deleteStrategyLocalOnly()`·`deleteStrategyFromGithub()`(성공 시) 전부 `renderAccountDropdown()` 추가 호출(`renderSettingsTab()`은 이미 있어 ①계좌관리 쪽은 자동 갱신됨). 계좌 자체 삭제(`deleteAccountV3`)는 기존에 이미 `renderAll()`(→`renderAccountDropdown()` 포함)을 호출하고 있어 별도 수정 불필요(커플링 대상 계좌가 사라지면 💍도 자연히 함께 사라짐, 해당 전략은 기존 "⚠️ 재지정 필요" 로직 그대로 노출)
- [x] Node vm 검증(신규 하네스 — top-level `let`을 `globalThis.__exports`로 브리지, GitHub Contents API를 흉내 내는 mock fetch로 409 시나리오까지 시뮬레이션, `closest()`를 지원하는 최소 fake DOM 트리 직접 구성):
  — 충돌 없음: PUT 1회로 성공
  — 1회 409 후 성공: 정확히 2회 PUT(재시도 시 최신 내용 재-GET), 다른 프로필(`s_existing`) 보존, 이번 프로필 내용이 최신값으로 갱신됨을 확인
  — 항상 409: 정확히 3회 시도(최초 1 + 재시도 2) 후 정확한 안내 문구로 실패
  — 유령 감지: 로컬에 없는 2개 id를 정확히 골라냄, 콘솔 HTML에 "파일에만 존재" 배지와 해당 id의 `removeGhostProfile('id')` 버튼 마크업 확인
  — 💍 표시: 커플링된 계좌만 `isAccountCoupled`가 true, ①계좌관리 목록·상단 드롭다운 옵션 텍스트 둘 다에서 커플링 계좌만 "💍 이름" 형태로 렌더됨을 정규식으로 확인
  — 행 클릭: 종목명 클릭→선택, 재클릭→해제, 입력칸/드래그핸들 클릭→선택 안 됨, group-hdr 행 클릭→선택 안 됨 — 5개 시나리오 전부 PASS
  — `node --check` 구문 오류 없음, CSS 중괄호 균형(depth 0) 확인
- [x] `git diff`(안전점 `091c409` 대비)에서 계산 함수 이름(getBasePrice/calcUnit/priceKRW/avgKRW/evalVal/currWeight/computeCashAmount/syncCashAmount/buildConsolidatedData/restoreFromDrive/sanitizeLoadedAppData/migrateAccount*/migrateStock*) 전부 grep 결과 없음(무수정) 확인 — 변경분 141줄이 위 4개 작업 범위에만 있음
- [x] sw.js CACHE_NAME rebalance-v64 → rebalance-v65
- [ ] **실제 브라우저·정식 주소 확인 필요**: 두 전략 연속 배포·409 재시도 실동작, 유령 프로필 실제 제거(사용자가 직접 수행 예정), 행 클릭 선택 시각 확인, 💍 표시 시각 확인 — 이번 세션은 Node vm 시뮬레이션까지만 수행(정식 운영 중인 index.html을 직접 수정했으므로 실제 브라우저 확인이 특히 중요)
